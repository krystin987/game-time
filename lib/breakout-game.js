...
Game.prototype.collisionBlockwall = function () {
  for(c=0; c<this.blockwall.blockColumnCount; c++) {
    for(r=0; r<this.blockwall.blockRowCount; r++) {
      ...
      if(b.status == 1) {
        if(this.ball.x > this.blockwall.x && this.ball.x < this.blockwall.x+this.blockwall.width && this.ball.y > this.blockwall.y && this.ball.y < this.blockwall.y+this.blockwall.height) {
          this.changeBallDY();
          b.status = 0;
          // score++;
      //     if(this.blockwall.blockRowCount*this.blockwall.blockColumnCount) {
